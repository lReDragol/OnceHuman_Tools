...
class GUI:
    ...
    def setup(self):
        with dpg.window(label="Main Window", width=1200, height=825):
            ...
            # Добавляем кнопки сохранения и загрузки конфигурации
        ...
        self.create_item_edit_window()  # Добавлено
    ...
    def show_item_config_window(self, item, item_type):
        ...
        with dpg.window(label=f"Настройка {item.name}", modal=True, show=True, tag=window_tag):
            ...
            # Добавляем ползунки для настройки
            ...
            # Если предмет является частью сета, отображаем описание сета
            set_id = item.set_id
            if set_id:
    ...
    def remove_item_from_slot(self, sender, app_data, user_data):
        ...
        dpg.add_button(label=f"Выберите {item_type_name}", callback=self.open_item_selection,
    ...
    def select_item_for_slot(self, sender, app_data, user_data):
        try:
            ...
            with dpg.item_handler_registry(tag=handler_tag) as handler_id:
                dpg.add_item_clicked_handler(button=dpg.mvMouseButton_Right, callback=self.open_item_config_window,
                                             user_data=item_type)
            dpg.bind_item_handler_registry(image_tag, handler_id)
            ...
            dpg.add_button(label=f"Выберите мод для {item_type.capitalize()}", callback=self.open_mod_selection,
    ...
    def open_mod_config_window(self, sender, app_data, user_data):
        ...
        with dpg.window(label=f"Настройка мода {mod['name']}", modal=True, show=True, tag=window_tag):
            ...
            # Отображаем характеристики мода
            if 'description' in mod:
                dpg.add_text(mod['description'])
    ...
    def mouse_down_callback(self, sender, app_data, user_data):
        ...
        self.context.mouse_pressed = True  # Синхронизируем с контекстом
        self.last_click_pos = dpg.get_mouse_pos(local=False)  # Сохраняем позицию клика

    def mouse_up_callback(self, sender, app_data, user_data):
        self.mouse_pressed = False
        self.context.mouse_pressed = False  # Синхронизируем с контекстом
    ...
    def update(self):
        self.context.update()  # Обновление контекста
        self.update_dps_display()
        self.update_damage_layer()
        self.update_stats_display()
