...
class Context:
    ...
    def process_hit(self, is_crit, is_weakspot):
        ...
        if is_crit:
            logging.debug("Crit hit detected, processing crit_hit event")
            self.mechanics_processor.process_weapon_event('crit_hit')
        if is_weakspot:
            logging.debug("Weakspot hit detected, processing weakspot_hit event")
    ...
    def display_damage_number(self, damage, hit_pos, is_crit, weakspot_hit, ability_name=None):
        ...
        if ability_name:
            ...
            if tex_id:
                ...
                # Если нет иконки, рисуем белый квадрат 10x10
    ...
    def trigger_ability(self, ability_name, **kwargs):
        ...
        if super_ability == 'unstable_bomber':
            ...
            # Расположим отрисовку урона
            center_pos = self.get_center_of_hit_area()
            self.display_damage_number(dmg, center_pos, False, False, ability_name='unstable_bomber')
            logging.debug("Unstable bomber damage displayed")
