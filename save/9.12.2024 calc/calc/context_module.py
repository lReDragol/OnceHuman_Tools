...
class Context:
    ...
    def update(self):
        ...
        # Обновляем временные баффы у игрока
        ...
        for item in self.scheduled_deletions[:]:
            ...
            if elapsed > item['duration']:
                ...
                try:
                    ...
                    # logging.info(f"Damage text {item['id']} moved to {new_pos} with alpha {alpha}.")
                except Exception as e:
    ...
    def display_damage_number(self, damage, hit_pos, is_crit, weakspot_hit, ability_name=None):
        ...
        # Сначала отрисовываем текст урона:
        dpg.draw_text(
            pos=[local_x, local_y],
            text=str(int(damage)),
            color=color,
        ...
        # Если есть ability_name и иконка для неё:
        if ability_name and ability_name in self.ability_icons:
            # Отобразим иконку слева от текста, например на 20px левее
            icon_x = local_x - 20
            icon_y = local_y
            # Иконка 100x100, но нам нужна 10x10, используем draw_image с масштабом:
            dpg.draw_image(self.ability_icons[ability_name],
                           pmin=[icon_x, icon_y],
    ...
    # =============== Новые методы для интеграции механик ===============
    ...
    def remove_buff(self, buff_name):
        # Если мы храним баффы в self.buffs, то удалим бафф
        if buff_name in self.buffs:
            # Отменяем эффекты баффа, если они были
            buff_data = self.buffs[buff_name]
            stat_bonuses = buff_data.get('stat_bonuses', {})
            # Уменьшаем статы обратно, если нужно
            # Здесь можно реализовать обратное действие
            for stat, val in stat_bonuses.items():
                # Считаем, что бафф просто давал бонус (увеличивал стат),
                # теперь его нужно убрать
    ...
    def trigger_ability(self, ability_name, **kwargs):
        super_ability = ability_name.lower()
        logging.info(f"Triggered ability: {ability_name} with {kwargs}")
        if super_ability == 'unstable_bomber':
            # Допустим, берём psi_intensity для расчёта
            psi_intensity = self.player.stats.get('psi_intensity', 100)
            dmg = psi_intensity
            # Наносим урон манекену
            self.mannequin.receive_damage(dmg)

            # Отобразим дополнительный урон с иконкой
            # Предположим, мы возьмём позицию центра области попадания для отображения:
            center_pos = self.get_center_of_hit_area()
            self.display_damage_number(dmg, center_pos, is_crit=False, weakspot_hit=False,
                                       ability_name='unstable_bomber')

            # Сбросим счетчик, если это нужно
    ...
    def apply_buff(self, buff_name, duration_bullets=None, stat_bonuses=None):
        if stat_bonuses:
            # Для простоты дадим бонусы на 5 секунд
            for stat, val in stat_bonuses.items():
    ...
    def consume_charge(self, effect_data):
        if self.charge_stacks > 0:
            ...
            # Можно применить эффект из effect_data, если нужно
        else:
    ...
    def check_custom_condition(self, condition):
        # Пример: всегда True, расширьте под реальную логику
        return True
