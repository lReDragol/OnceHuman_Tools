...
class Item:
    ...
    def get_stats(self):
        stats = {}
        # Получаем данные для текущего количества звёзд
        star_data = self.base_stats_data.get(str(self.star), {})
        # Получаем данные уровней для текущих звёзд
        ...
        # Копируем характеристики текущего уровня
        stats = level_data.copy()

        # Применяем бонусы калибровки
...
class Weapon:
    def __init__(self, weapon_data):
        ...
        self.base_stats = weapon_data['base_stats']
        self.mechanics = weapon_data.get('mechanics', {})
...
class Player:
    ...
    def apply_stat_bonus(self, stat, value, duration):
