...
from player import Item, Weapon
import shutil
import os
from mechanics import MechanicsProcessor
import logging
...
class DamageCalculator:
    ...
    def apply_weapon_and_status_bonus(self, damage):
        ...
        logging.info(f"Weapon and status bonus applied. Total bonus: {total_bonus * 100}%. Damage now: {damage}")
        return damage
    ...
    def apply_crit_bonus(self, damage):
        ...
        if is_crit:
            ...
            # Обрабатываем события критического попадания
            self.context.mechanics_processor.process_weapon_event('crit_hit')
    ...
    def apply_weakspot_bonus(self, damage):
        ...
        # Обрабатываем события попадания в слабую точку
        self.context.mechanics_processor.process_weapon_event('weakspot_hit')
...
class Context:
    ...
    def try_fire_weapon(self):
        ...
        if current_time - self.last_fire_time < time_between_shots:
            ...
            return  # Выходим из метода, если ещё не прошло достаточно времени
        self.last_fire_time = current_time  # Обновляем время последнего выстрела
        # Продолжаем обработку выстрела
        # Получаем количество снарядов за выстрел
        projectiles_per_shot = int(self.player.stats.get('projectiles_per_shot', 1))
        for _ in range(projectiles_per_shot):
            # Получаем текущую позицию мыши
            current_mouse_pos = dpg.get_mouse_pos(local=False)
            # Симулируем попадание каждой дробинки
            hit_pos = self.simulate_projectile_hit(current_mouse_pos)
            if not self.is_within_target_area(hit_pos):
                continue  # Пропускаем, если промахнулись
            ...
            self.process_hit(is_crit, weakspot_hit)
        self.current_ammo -= 1
        # Обрабатываем события
        self.mechanics_processor.process_weapon_event('on_fire')
        logging.info(f"Weapon fired. Ammo left: {self.current_ammo}")
    ...
    def is_within_target_area(self, mouse_pos):
        ...
        if ((normal_zone['left'] <= local_x <= normal_zone['right'] and normal_zone['top'] <= local_y <= normal_zone[
            'bottom']) or
                (weakspot_rect['left'] <= local_x <= weakspot_rect['right'] and weakspot_rect['top'] <= local_y <=
                 weakspot_rect['bottom'])):
            return True
        else:
            return False
    ...
    def is_weakspot_hit(self, mouse_pos):
        ...
        local_x = x - window_pos[0]
        local_y = y - window_pos[1]
        if weakspot_rect['left'] <= local_x <= weakspot_rect['right'] and weakspot_rect['top'] <= local_y <= \
                weakspot_rect['bottom']:
            return True
        else:
            return False
    ...
    def display_unstable_bomber_damage(self, damage):
        # Отображаем урон в центре синей зоны розовым цветом
        normal_zone = {
            'left': 90,
            'right': 210,
            'top': 50,
        ...
        local_y = y_center
        damage_text_id = dpg.generate_uuid()
        color = [255, 105, 180, 255]  # Розовый цвет
        current_time = time.time()
        dpg.draw_text(
            pos=[local_x, local_y],
            text=str(int(damage)),
        ...
        self.scheduled_deletions.append({
            ...
            'duration': 1.0,
            'start_pos': [local_x, local_y],
            'velocity': (0, -50),  # Движение вверх со скоростью 50 пикселей/сек
            'color': color
        })
        # Логируем нанесённый урон
        logging.info(f"Unstable Bomber dealt {damage} damage.")
    ...
    def get_set_by_id(self, set_id):
        for game_set in self.sets_data:
            print(f"Processing game_set: {game_set}")
            if 'id' not in game_set:
                print("Error: 'id' key not found in game_set")
                continue
