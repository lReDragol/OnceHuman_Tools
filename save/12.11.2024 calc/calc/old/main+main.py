...
def edit_mods(sender, app_data, user_data):
    ...
    # Добавляем моды оружия отдельно
    if 'mod_weapon' in mods_data:
        mods = mods_data['mod_weapon']
        if mods:
            dpg.add_text("Категория: Оружие", parent="mods_list")
            for mod in mods:
                mod_name = mod['name']
                with dpg.group(horizontal=True, parent="mods_list"):
                    dpg.add_button(label=mod_name, callback=select_mod, user_data=mod)
...
def handle_events(is_crit):
    ...
    # В вашем примере можно считать, что Unstable Bomber срабатывает при крите
    if is_crit:
...
def open_mod_selection(sender, app_data, user_data):
    ...
    # Если категория "Оружие", загружаем моды оружия отдельно
    if category == "Оружие":
                ...
                with dpg.child_window(width=600, height=650, horizontal_scrollbar=True):
                    ...
                    with dpg.table(header_row=False, resizable=False, policy=dpg.mvTable_SizingFixedFit):
                        ...
                        with dpg.table_row():
                            ...
                            dpg.add_input_int(default_value=1000, min_value=0, max_value=100000, step=0,
