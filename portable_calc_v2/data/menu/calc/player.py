# player.py

import time
import copy
import logging
import re
import math
from .mechanics import add_stats, check_conditions, normalize_effects

logging.basicConfig(level=logging.INFO)

class Item:
    def __init__(self, item_data, base_stats, calibration_bonuses):
        self.id = item_data['id']
        self.name = item_data['name']
        self.type = item_data['type']
        self.rarity = item_data['rarity']
        self.set_id = item_data.get('set_id')
        self.description = item_data.get('description', '')
        self.effects = copy.deepcopy(item_data.get('effects', []))
        self.star = 1
        self.level = 1
        self.calibration = 0
        self.base_stats_data = base_stats.get(self.type, {}).get(self.rarity, {}).get('stars', {})
        if not self.base_stats_data:
            raise ValueError(f"No base stats for type '{self.type}' and rarity '{self.rarity}'")
        self.max_stars = {
            'legendary': 6,
            'epic': 5,
            'rare': 4,
            'common': 3
        }[self.rarity]
        self.calibration_bonuses = calibration_bonuses

    def calculate_stats(self):
        stats = {}
        add_stats(stats, self.base_stats)
        star_multipliers = {
            1: 1.0,
            2: 1.1,
            3: 1.2,
            4: 1.3,
            5: 1.4,
            6: 1.5
        }
        level_multipliers = {
            1: 1.0,
            2: 1.05,
            3: 1.10,
            4: 1.15,
            5: 1.20
        }
        calibration_bonuses = {
            0: 0.0,
            1: 0.02,
            2: 0.04,
            3: 0.06,
            4: 0.08,
            5: 0.10,
            6: 0.12
        }
        star_multiplier = star_multipliers.get(self.star, 1.0)
        level_multiplier = level_multipliers.get(self.level, 1.0)
        calibration_bonus = calibration_bonuses.get(self.calibration, 0.0)
        for stat in stats:
            base_value = stats[stat]
            if isinstance(base_value, (int, float)):
                new_value = base_value * star_multiplier * level_multiplier * (1 + calibration_bonus)
                if stat in ['projectiles_per_shot', 'magazine_capacity', 'some_other_integer_stat']:
                    stats[stat] = int(round(new_value))
                else:
                    stats[stat] = new_value
        return stats

    def get_stats(self):
        stats = {}
        star_data = self.base_stats_data.get(str(self.star), {})
        levels_data = star_data.get('levels', {})
        level_data = levels_data.get(str(self.level), {})

        if not level_data:
            raise ValueError(f"No data for level {self.level} for item '{self.name}' at star level {self.star}")

        stats = level_data.copy()
        for stat_name in ['hp', 'psi_intensity', 'pollution_resist']:
            base_value = stats.get(stat_name, 0)
            calibration_bonus = self.calibration_bonuses.get(stat_name, {}).get(str(self.calibration), 0.0)
            stats[stat_name] = round(base_value * (1 + calibration_bonus))
        return stats

    def get_max_calibration(self):
        if self.level in [1, 2]:
            return 2
        elif self.level in [3, 4]:
            return 4
        elif self.level == 5:
            return 6
        else:
            return 0

class Player:
    def __init__(self):
        self.base_stats = {}
        self.stats = {}
        self.equipped_items = {}
        self.equipped_mods = {}
        self.weapon = None
        self.active_stat_bonuses = {}
        self.context = None
        self.mod_effects = []
        self.set_effects = []
        self.static_effects = []
        self.active_set_bonuses = []
        self.event_effect_sources = []
        self.effect_sources_dirty = True
        self.recalculate_stats()

    def equip_item(self, item):
        self.equipped_items[item.type] = item
        self.effect_sources_dirty = True
        self.recalculate_stats()

    def remove_item(self, item_type):
        if item_type in self.equipped_items:
            del self.equipped_items[item_type]
            self.effect_sources_dirty = True
            self.recalculate_stats()

    def equip_mod(self, mod, item_type):
        self.equipped_mods[item_type] = mod
        self.effect_sources_dirty = True
        self.recalculate_stats()

    def remove_mod(self, item_type):
        if item_type in self.equipped_mods:
            del self.equipped_mods[item_type]
            self.effect_sources_dirty = True
            self.recalculate_stats()

    def equip_weapon(self, weapon):
        self.weapon = weapon
        self.effect_sources_dirty = True
        self.recalculate_stats()

    def remove_weapon(self):
        self.weapon = None
        self.effect_sources_dirty = True
        self.recalculate_stats()

    def refresh_effect_sources(self, context=None):
        context = context or self.context
        self.mod_effects = []
        self.set_effects = []
        self.active_set_bonuses = []
        self.event_effect_sources = []
        self.static_effects = []

        effect_groups = []
        for item in self.equipped_items.values():
            effects = normalize_effects(getattr(item, 'effects', []) or [])
            if effects:
                effect_groups.append({
                    'source': f"item:{item.id}",
                    'effects': effects,
                })

        for mod in self.equipped_mods.values():
            effects = normalize_effects(mod.get('effects', []))
            if context and hasattr(context, 'build_mod_secondary_effects'):
                effects.extend(normalize_effects(context.build_mod_secondary_effects(mod)))
            self.mod_effects.extend(effects)
            effect_groups.append({
                'source': f"mod:{mod.get('name', 'unknown')}",
                'effects': effects,
            })

        if self.weapon:
            for slot_name, attachment in sorted(self.weapon.equipped_attachments.items()):
                effects = normalize_effects(attachment.get('effects', []))
                if not effects:
                    continue
                effect_groups.append({
                    'source': f"attachment:{slot_name}:{attachment.get('id', attachment.get('name', 'unknown'))}",
                    'effects': effects,
                })

        if context:
            set_counts = {}
            for item in self.equipped_items.values():
                if item.set_id:
                    set_counts[item.set_id] = set_counts.get(item.set_id, 0) + 1

            for set_id, count in set_counts.items():
                game_set = context.get_set_by_id(set_id)
                if not game_set:
                    continue
                for bonus in sorted(game_set.get('bonuses', []), key=lambda entry: entry.get('required_items', 0)):
                    if count >= bonus.get('required_items', 0):
                        bonus_effects = normalize_effects(bonus.get('effects', []))
                        self.active_set_bonuses.append({
                            'set_id': set_id,
                            'set_name': game_set.get('name', set_id),
                            'required_items': bonus.get('required_items', 0),
                            'effects': bonus_effects,
                        })
                        self.set_effects.extend(bonus_effects)
                        effect_groups.append({
                            'source': f"set:{set_id}:{bonus.get('required_items', 0)}",
                            'effects': bonus_effects,
                        })

        for group in effect_groups:
            on_event_effects = [effect for effect in group['effects'] if effect.get('type') == 'on_event']
            static_effects = [effect for effect in group['effects'] if effect.get('type') != 'on_event']
            if on_event_effects:
                self.event_effect_sources.append({
                    'source': group['source'],
                    'effects': on_event_effects,
                })
            self.static_effects.extend(static_effects)

        if self.weapon and isinstance(self.weapon.mechanics_data, dict):
            weapon_effects = normalize_effects(self.weapon.mechanics_data.get('effects', []))
            self.static_effects.extend([effect for effect in weapon_effects if effect.get('type') != 'on_event'])

        if context and hasattr(context, 'mechanics_processor'):
            context.mechanics_processor.set_external_mechanics(self.event_effect_sources)

        self.effect_sources_dirty = False

    def recalculate_stats(self, context=None):
        context = context or self.context
        if self.effect_sources_dirty:
            self.refresh_effect_sources(context)

        self.stats = self.base_stats.copy()
        if self.weapon:
            weapon_stats = self.weapon.get_stats()
            add_stats(self.stats, weapon_stats)
        for item in self.equipped_items.values():
            item_stats = item.get_stats()
            add_stats(self.stats, item_stats)

        for effect in self.static_effects:
            self.process_effect(effect, context)

        for stat, bonuses in self.active_stat_bonuses.items():
            self.stats[stat] = self.stats.get(stat, 0) + sum(bonus['value'] for bonus in bonuses)

        self.apply_derived_stats()

    def process_effect(self, effect, context=None):
        effect_type = effect.get('type')
        if effect_type == 'increase_stat':
            self.apply_stat_delta(effect.get('stat'), effect.get('value', 0), context=context)
        elif effect_type == 'decrease_stat':
            self.apply_stat_delta(effect.get('stat'), effect.get('value', 0), context=context, negative=True)
        elif effect_type == 'set_flag':
            flags = effect.get('flag')
            values = effect.get('value')
            alias_map = {
                'super_armor': 'has_super_armor',
            }
            if isinstance(flags, list):
                if not isinstance(values, list):
                    values = [values] * len(flags)
                for flag, flag_value in zip(flags, values):
                    flag = alias_map.get(flag, flag)
                    self.stats[flag] = flag_value
            elif flags:
                flags = alias_map.get(flags, flags)
                self.stats[flags] = values
        elif effect_type == 'passive_effect':
            conditions = effect.get('conditions', {})
            effect_context = context.build_effect_context() if context and hasattr(context, 'build_effect_context') else {'mechanics_context': context}
            if conditions and not check_conditions(conditions, effect_context):
                return
            effect_name = effect.get('effect_name')
            passive_stat_aliases = {
                'bounce_base_damage_bonus': {
                    'damage_bonus_percent': 'bounce_damage_percent',
                    'damage_percent': 'bounce_damage_percent',
                },
                'bounce_damage_bonus': {
                    'damage_bonus_percent': 'bounce_damage_percent',
                    'damage_percent': 'bounce_damage_percent',
                },
                'frost_dmg_bonus': {
                    'damage_bonus_percent': 'frost_elemental_damage_percent',
                },
                'shrapnel_dmg_bonus': {
                    'damage_bonus_percent': 'shrapnel_damage_percent',
                },
                'shrapnel_crit_dmg_bonus': {
                    'crit_dmg_bonus_percent': 'shrapnel_crit_damage_percent',
                },
                'heavy_armor': {
                    'weapon_dmg_percent': 'weapon_damage_percent',
                },
                'fortress_warfare_damage_boost': {
                    'weapon_damage_bonus_percent': 'weapon_damage_percent',
                },
                'weakspot_damage_bonus': {
                    'weakspot_damage_bonus_percent': 'weakspot_damage_percent',
                },
                'power_surge_base_factor_bonus': {
                    'trigger_factor_bonus_percent': 'shock_trigger_chance_percent',
                },
                'power_surge_damage_factor': {
                    'damage_factor_bonus_percent': 'power_surge_damage_percent',
                },
                'unstable_bomber_crit_dmg': {
                    'guaranteed_crit_dmg': 'unstable_bomber_guaranteed_crit',
                },
            }
            properties = effect.get('properties', {})
            if effect_name == 'fortress_warfare_scaling_attack' and context:
                active_seconds = context.get_status_uptime('fortress_warfare')
                max_bonus = float(properties.get('max_attack_bonus_percent', 0) or 0)
                scaling_rate = float(properties.get('scaling_rate_per_second', 0) or 0)
                if active_seconds > 0 and max_bonus > 0 and scaling_rate > 0:
                    self.apply_stat_delta('weapon_damage_percent', min(max_bonus, active_seconds * scaling_rate), context=context)
                return
            if effect_name == 'unstable_bomber_dmg_bonus':
                scale = float(properties.get('dmg_bonus_percent_per_weakspot_dmg_percent', 0) or 0)
                if scale:
                    self.apply_stat_delta('unstable_bomber_damage_percent', self.stats.get('weakspot_damage_percent', 0) * scale, context=context)
                return
            if effect_name == 'unstable_bomber_delay_bonus':
                self.apply_stat_delta('unstable_bomber_delay_bonus_seconds', float(properties.get('delay_bonus_seconds', 0) or 0), context=context)
                return
            if effect_name == 'unstable_bomber_dmg_scaling':
                self.apply_stat_delta('unstable_bomber_damage_per_delay_step_percent', float(properties.get('dmg_scaling_per_0.1s_delay', 0) or 0), context=context)
                return
            for stat, value in effect.get('properties', {}).items():
                stat = passive_stat_aliases.get(effect_name, {}).get(stat, stat)
                if isinstance(value, (int, float)):
                    self.apply_stat_delta(stat, value, context=context)
                elif isinstance(value, bool):
                    self.stats[stat] = value
        elif effect_type == 'conditional_stat_bonus':
            if not context:
                return
            effect_context = context.build_effect_context() if hasattr(context, 'build_effect_context') else {'mechanics_context': context}
            condition_key = effect.get('condition')
            condition_value = effect.get('value', 0)
            if condition_key and not check_conditions({condition_key: condition_value}, effect_context):
                return
            increment = effect.get('increment')
            for stat_name, stat_value in iter_effect_stat_pairs(effect):
                delta = increment if increment is not None else stat_value
                self.apply_stat_delta(stat_name, delta, context=context)
        elif effect_type == 'conditional_effect':
            if self.evaluate_condition(effect.get('condition'), context):
                for nested_effect in normalize_effects(effect.get('effects', [])):
                    self.process_effect(nested_effect, context)
        elif effect_type == 'modify_skill':
            skill_name = str(effect.get('skill') or '').lower()
            is_active = False
            if context and skill_name:
                is_active = context.is_status_active(skill_name) or context.current_mode == skill_name
                if skill_name == 'the_bulls_eye':
                    is_active = is_active or context.is_status_active('bulls_eye')
            if is_active:
                for nested_effect in normalize_effects(effect.get('effects', [])):
                    self.process_effect(nested_effect, context)
        elif effect_type == 'frost_vortex_applies':
            if context and (context.is_status_active('frostbite') or context.is_status_active('frost_vortex')):
                for nested_effect in normalize_effects(effect.get('effects', [])):
                    if nested_effect.get('type') == 'apply_status':
                        continue
                    self.process_effect(nested_effect, context)
        elif effect_type == 'out_of_combat':
            if context and context.is_out_of_combat():
                elapsed = context.get_out_of_combat_time()
                for nested_effect in normalize_effects(effect.get('effects', [])):
                    if nested_effect.get('type') == 'gain_stack' and nested_effect.get('effects'):
                        every_seconds = max(float(nested_effect.get('every_seconds', 1) or 1), 1.0)
                        max_stacks = nested_effect.get('max_stacks')
                        stacks = int(elapsed // every_seconds)
                        if max_stacks is not None:
                            stacks = min(stacks, int(max_stacks))
                        if stacks > 0:
                            self.apply_scaled_effects(nested_effect, stacks, context)
                    else:
                        self.process_effect(nested_effect, context)
        elif effect_type == 'while_active':
            active_status = effect.get('status')
            if context and context.is_status_active(active_status):
                for nested_effect in normalize_effects(effect.get('effects', [])):
                    prepared_effect = copy.deepcopy(nested_effect)
                    prepared_effect.setdefault('_active_status', active_status)
                    self.process_effect(prepared_effect, context)
        elif effect_type == 'more_targets_hit':
            if context:
                target_count = max(0, int(round(self.stats.get('bounce_targets', 0))))
                if target_count > 0:
                    for nested_effect in normalize_effects(effect.get('effects', [])):
                        scaled_effect = copy.deepcopy(nested_effect)
                        value = scaled_effect.get('value')
                        if isinstance(value, (int, float)):
                            scaled_effect['value'] = min(float(value), target_count * 15.0)
                        self.process_effect(scaled_effect, context)
        elif effect_type == 'per_stack':
            if context:
                stack_count = context.get_stack_count(effect.get('stack_source'))
                if stack_count > 0:
                    self.apply_scaled_effects(effect, stack_count, context)
        elif effect_type == 'per_hits_taken':
            if context:
                hits_required = max(int(effect.get('hits_required', 1) or 1), 1)
                stacks = int(getattr(context, 'hits_taken_count', 0) // hits_required)
                if stacks > 0:
                    self.apply_scaled_effects(effect, stacks, context)
        elif effect_type == 'per_hp_loss':
            if context:
                every_percent = max(effect.get('every_percent', 0), 1)
                hp_lost_percent = max(0.0, 100.0 - context.get_player_hp_ratio() * 100.0)
                stacks = int(hp_lost_percent // every_percent)
                if stacks > 0:
                    self.apply_scaled_effects(effect, stacks, context)
        elif effect_type == 'per_meter':
            if context:
                distance_over_threshold = max(0, int(math.floor(context.target_distance - 20)))
                if distance_over_threshold > 0:
                    self.apply_scaled_effects(effect, distance_over_threshold, context)
        elif effect_type == 'per_bullets_consumed':
            if context:
                bullets = max(effect.get('bullets', 0), 1)
                stacks = context.magazine_bullets_fired // bullets
                if stacks > 0:
                    self.apply_scaled_effects(effect, stacks, context)
        elif effect_type == 'per_player_in_fortress':
            if context:
                players_in_fortress = max(1, int(getattr(context, 'players_in_fortress', 1)))
                self.apply_scaled_effects(effect, players_in_fortress, context)
        elif effect_type == 'per_weakspot_hit_rate':
            if context:
                every_percent = max(effect.get('every_percent', 0), 1)
                stacks = int(context.last_magazine_weakspot_rate // every_percent)
                if stacks > 0:
                    self.apply_scaled_effects(effect, stacks, context)
        elif effect_type == 'per_stat':
            stat_name = effect.get('stat')
            step = max(effect.get('value', 0), 1)
            stat_value = self.stats.get(stat_name, 0)
            stacks = int(stat_value // step)
            if stacks > 0:
                self.apply_scaled_effects(effect, stacks, context)
        elif effect_type == 'increase_stat_over_time':
            if context:
                active_status = effect.get('_active_status') or effect.get('status') or context.current_mode
                active_seconds = context.get_status_uptime(active_status)
                if active_seconds > 0:
                    stat_name = effect.get('stat')
                    bonus_value = active_seconds * float(effect.get('value_per_second', 0))
                    max_value = effect.get('max_value')
                    if max_value is not None:
                        bonus_value = min(float(max_value), bonus_value)
                    if stat_name and bonus_value:
                        self.apply_stat_delta(stat_name, bonus_value)
        elif effect_type == 'special_effect':
            effect_name = effect.get('effect_name')
            value = float(effect.get('value', 0) or 0)
            if effect_name == 'shrapnel_weakspot_hit_chance_increase':
                self.apply_stat_delta('shrapnel_weakspot_hit_chance_percent', value, context=context)
            elif effect_name in {'frost_vortex_damage_increase', 'frost_vortex_ultimate_damage_increase'}:
                bonus_value = value
                if context:
                    if effect.get('based_on') == 'duration':
                        bonus_value = min(value, context.get_status_remaining('frost_vortex') * 10.0)
                    elif effect.get('based_on') == 'enemies_inside':
                        bonus_value = min(value, max(1, context.enemies_within_distance) * 10.0)
                self.apply_stat_delta('frost_vortex_damage_percent', bonus_value, context=context)

    def apply_scaled_effects(self, effect, stacks, context=None):
        for nested_effect in normalize_effects(effect.get('effects', [])):
            scaled_effect = copy.deepcopy(nested_effect)
            value = scaled_effect.get('value')
            max_value = scaled_effect.get('max_value')
            if isinstance(value, list):
                scaled_values = [entry * stacks for entry in value]
                if max_value is not None:
                    scaled_values = [max(-max_value, min(max_value, scaled)) for scaled in scaled_values]
                scaled_effect['value'] = scaled_values
            elif isinstance(value, (int, float)):
                scaled_value = value * stacks
                if max_value is not None:
                    scaled_value = max(-max_value, min(max_value, scaled_value))
                scaled_effect['value'] = scaled_value
            self.process_effect(scaled_effect, context)

    def apply_stat_delta(self, stat_name, value, context=None, negative=False):
        if isinstance(stat_name, list):
            if not isinstance(value, list):
                value = [value] * len(stat_name)
            for single_stat, single_value in zip(stat_name, value):
                self.apply_stat_delta(single_stat, single_value, context=context, negative=negative)
            return

        if not stat_name:
            return

        alias_map = {
            'weapon_damage_bonus_percent': 'weapon_damage_percent',
            'weapon_dmg_percent': 'weapon_damage_percent',
            'weapon_dmg_bonus_percent': 'weapon_damage_percent',
            'damage_bonus_percent': 'weapon_damage_percent',
            'damage_percent': 'weapon_damage_percent',
            'damage_percent_increase': 'weapon_damage_percent',
            'super_armor': 'has_super_armor',
            'attack': 'damage_per_projectile',
            'attack_percent': 'weapon_damage_percent',
            'attack_bonus_percent': 'weapon_damage_percent',
            'damage_percent': 'weapon_damage_percent',
            'fire_rate_bonus_percent': 'fire_rate_percent',
            'crit_rate_bonus_percent': 'crit_rate_percent',
            'crit_dmg_bonus_percent': 'crit_damage_percent',
            'crit_dmg_percent': 'crit_damage_percent',
            'shrapnel_crit_dmg_bonus_percent': 'crit_damage_percent',
            'weakspot_dmg_percent': 'weakspot_damage_percent',
            'weakspot_dmg_bonus_percent': 'weakspot_damage_percent',
            'weakspot_damage_bonus_percent': 'weakspot_damage_percent',
            'reload_efficiency_percent': 'reload_speed_percent',
            'power_surge_status_duration_percent': 'power_surge_duration_percent',
            'bomb_ultimate_damage_percent': 'unstable_bomber_final_damage_percent',
            'explosive_final_dmg_percent': 'unstable_bomber_final_damage_percent',
            'burn_dmg_percent': 'burn_damage_percent',
            'frost_dmg_percent': 'frost_elemental_damage_percent',
            'shrapnel_dmg_percent': 'shrapnel_damage_percent',
            'magazine_capacity_bonus': 'magazine_capacity',
            'range_percent': 'range',
            'weapon_damage_percent_second_half_mag': 'weapon_damage_percent',
            'marked_enemies_damage_vs_metas_percent': 'marked_target_damage_percent',
        }
        stat_name = alias_map.get(stat_name, stat_name)

        delta = -value if negative else value
        if stat_name == 'hp_percent_bonus':
            base_hp = context.base_hp if context and hasattr(context, 'base_hp') else self.stats.get('hp', 0)
            self.stats['hp'] = self.stats.get('hp', 0) + base_hp * delta / 100.0
            return
        if stat_name == 'psi_intensity_percent_bonus':
            base_psi = self.stats.get('psi_intensity', 0)
            self.stats['psi_intensity'] = self.stats.get('psi_intensity', 0) + base_psi * delta / 100.0
            return
        self.stats[stat_name] = self.stats.get(stat_name, 0) + delta

    def evaluate_condition(self, condition, context=None):
        if not condition:
            return True
        if context is None:
            return False

        normalized = condition.strip().lower()
        if normalized == 'target_is_marked':
            return context.is_status_active('the_bulls_eye') or context.is_status_active('bulls_eye')
        if normalized == 'enemy_has_burn':
            return context.is_status_active('burn')
        if normalized == 'enemy_without_power_surge':
            return not context.is_status_active('power_surge')

        ratio_conditions = {
            r'^hp / max_hp ([<>]=?) ([0-9.]+)$': context.get_player_hp_ratio(),
            r'^target_hp / target_max_hp ([<>]=?) ([0-9.]+)$': (
                context.mannequin.current_hp / context.mannequin.max_hp if context.mannequin.max_hp else 1.0
            ),
            r'^enemies_within_distance ([<>]=?|==) ([0-9.]+)$': context.enemies_within_distance,
            r'^target_distance ([<>]=?|==) ([0-9.]+)$': context.target_distance,
        }
        for pattern, current_value in ratio_conditions.items():
            match = re.match(pattern, normalized)
            if match:
                operator = match.group(1)
                threshold = float(match.group(2))
                return self.compare_values(current_value, threshold, operator)
        return False


    @staticmethod
    def compare_values(current_value, threshold, operator):
        if operator == '>':
            return current_value > threshold
        if operator == '>=':
            return current_value >= threshold
        if operator == '<':
            return current_value < threshold
        if operator == '<=':
            return current_value <= threshold
        if operator == '==':
            return current_value == threshold
        return False

    def apply_derived_stats(self):
        if 'magazine_capacity_percent' in self.stats and 'magazine_capacity' in self.stats:
            self.stats['magazine_capacity'] = int(round(
                self.stats['magazine_capacity'] * (1 + self.stats['magazine_capacity_percent'] / 100.0)
            ))
        if 'fire_rate_percent' in self.stats and 'fire_rate' in self.stats:
            self.stats['fire_rate'] = self.stats['fire_rate'] * (1 + self.stats['fire_rate_percent'] / 100.0)
        if 'reload_speed_percent' in self.stats and 'reload_time_seconds' in self.stats and self.stats['reload_time_seconds'] > 0:
            self.stats['reload_time_seconds'] = self.stats['reload_time_seconds'] / (
                1 + self.stats['reload_speed_percent'] / 100.0
            )
        if 'raise_weapon_speed_percent' in self.stats and 'ads_time_seconds' in self.stats and self.stats['ads_time_seconds'] > 0:
            self.stats['ads_time_seconds'] = self.stats['ads_time_seconds'] / (
                1 + self.stats['raise_weapon_speed_percent'] / 100.0
            )

        weapon_type = getattr(self.weapon, 'type', None)
        if weapon_type:
            for stat_suffix in ('accuracy', 'stability'):
                class_stat_name = f'{weapon_type}_{stat_suffix}'
                if class_stat_name in self.stats:
                    self.stats[stat_suffix] = self.stats.get(stat_suffix, 0) + self.stats.get(class_stat_name, 0)

        if 'ads_stability_percent' in self.stats:
            self.stats['stability'] = self.stats.get('stability', 0) + self.stats.get('ads_stability_percent', 0)

        base_max_stamina = 100.0
        self.stats['max_stamina'] = base_max_stamina * (1 + self.stats.get('max_stamina_percent', 0) / 100.0)

        for flag_name in ['has_super_armor', 'can_deal_weakspot_damage', 'is_invincible', 'can_crit']:
            if flag_name in self.stats and isinstance(self.stats[flag_name], (int, float)):
                self.stats[flag_name] = self.stats[flag_name] > 0

    def apply_stat_bonus(self, stat, value, duration, max_stacks=None, source=None):
        current_time = time.time()
        if stat not in self.active_stat_bonuses:
            self.active_stat_bonuses[stat] = []
        bonuses = self.active_stat_bonuses[stat]

        if max_stacks is not None:
            matching_bonuses = [
                bonus for bonus in bonuses
                if source is None or str(bonus.get('source', '')) == str(source)
            ]
            if len(matching_bonuses) >= max_stacks:
                oldest_bonus = min(matching_bonuses, key=lambda bonus: bonus['end_time'])
                oldest_bonus['end_time'] = max(oldest_bonus['end_time'], current_time + duration)
                self.recalculate_stats()
                return False

        bonuses.append({
            'value': value,
            'end_time': current_time + duration,
            'source': source,
        })
        self.recalculate_stats()
        return True

    def get_source_bonus_total(self, stat, source_prefix):
        if not source_prefix:
            return 0
        total = 0
        for bonus in self.active_stat_bonuses.get(stat, []):
            bonus_source = str(bonus.get('source', ''))
            if bonus_source == source_prefix or bonus_source.startswith(f"{source_prefix}:"):
                total += bonus.get('value', 0)
        return total

    def remove_stat_bonus_source(self, source_prefix):
        if not source_prefix:
            return 0
        removed = 0
        updated = False
        for stat, bonuses in list(self.active_stat_bonuses.items()):
            remaining = []
            for bonus in bonuses:
                bonus_source = str(bonus.get('source', ''))
                if bonus_source == source_prefix or bonus_source.startswith(f"{source_prefix}:"):
                    removed += 1
                    updated = True
                else:
                    remaining.append(bonus)
            if remaining:
                self.active_stat_bonuses[stat] = remaining
            else:
                del self.active_stat_bonuses[stat]
        if updated:
            self.recalculate_stats()
        return removed

    def update_active_stat_bonuses(self):
        current_time = time.time()
        updated = False
        for stat, bonuses in list(self.active_stat_bonuses.items()):
            for bonus in bonuses[:]:
                if current_time >= bonus['end_time']:
                    bonuses.remove(bonus)
                    updated = True
            if not bonuses:
                del self.active_stat_bonuses[stat]
                updated = True
        if updated:
            self.recalculate_stats()


def iter_effect_stat_pairs(effect):
    stats = effect.get('stat')
    values = effect.get('value', 0)
    if isinstance(stats, list):
        if isinstance(values, list):
            return list(zip(stats, values))
        return [(stat, values) for stat in stats]
    return [(stats, values)]


class Mannequin:
    def __init__(self):
        self.max_hp = 100000
        self.current_hp = self.max_hp
        self.status = 'normal'
        self.enemy_type = 'Обычный'
        self.effects = {}
        self.show_hotbar = True
        self.show_unified_shotgun_damage = False

    def apply_effect(self, effect_name):
        self.effects[effect_name] = True
        logging.debug(f"Effect {effect_name} applied to mannequin.")

    def remove_effect(self, effect_name):
        if effect_name in self.effects:
            del self.effects[effect_name]
            logging.debug(f"Effect {effect_name} removed from mannequin.")

    def set_hp(self, hp):
        self.max_hp = hp
        self.current_hp = min(self.current_hp, hp)
        logging.debug(f"Mannequin HP set to {self.max_hp}.")

    def toggle_hotbar(self):
        self.show_hotbar = not self.show_hotbar
        logging.debug(f"Mannequin hotbar toggled to {self.show_hotbar}.")

    def toggle_unified_shotgun_damage(self):
        self.show_unified_shotgun_damage = not self.show_unified_shotgun_damage
        logging.debug(f"Unified shotgun damage toggled to {self.show_unified_shotgun_damage}.")

    def set_status(self, status):
        self.status = status
        logging.debug(f"Mannequin status set to {self.status}.")

    def set_enemy_type(self, enemy_type):
        self.enemy_type = enemy_type
        logging.debug(f"Mannequin enemy type set to {self.enemy_type}.")

    def receive_damage(self, damage):
        self.current_hp -= damage
        self.current_hp = max(self.current_hp, 0)
        logging.debug(f"Mannequin received {damage} damage. HP: {self.current_hp}/{self.max_hp}.")

    def apply_status(self, status_name, duration, **kwargs):
        logging.debug(f"Mannequin received status {status_name} for {duration}s with {kwargs}.")

    def update_effects(self, delta_time):
        effects_to_remove = []
        for effect_name, effect_data in self.effects.items():
            if isinstance(effect_data, dict) and 'duration' in effect_data:
                effect_data['duration'] -= delta_time
                if effect_data['duration'] <= 0:
                    effects_to_remove.append(effect_name)
        for effect_name in effects_to_remove:
            self.remove_effect(effect_name)

def add_stats(stats_dict, new_stats):
    for stat, value in new_stats.items():
        if isinstance(value, dict):
            if stat not in stats_dict:
                stats_dict[stat] = {}
            add_stats(stats_dict[stat], value)
        elif isinstance(value, (int, float)):
            current_value = stats_dict.get(stat, 0)
            stats_dict[stat] = current_value + value
        else:
            pass
