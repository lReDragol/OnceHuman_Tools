...
def capture_zone_screenshot():
    screenshot_path = os.path.join(tempfile.gettempdir(), "once_human_zone_capture.png")
    _clear_clipboard()
...
class ScreenshotSelector(QMainWindow):
    def __init__(self, image_path, callback, config, ui_text=None, result_holder=None):
        super().__init__()
    ...
    def save_config(self):
        existing_zones = self.config.bot_fish_tab.get("zones", [])
        ...
        self.config.bot_fish_tab["zones"] = existing_zones
...
def start_zone_creation(config, callback=None, ui_text=None):
